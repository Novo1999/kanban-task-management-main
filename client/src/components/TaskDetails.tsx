...
export type UseGetTask = {
  data: {
    data: {
      title: string, subtasks: Array<{
        _id: string, status: string, name: string
  ...
  return (
    <Overlay>
      {isTaskLoading ? (
        ...
        <div ref={taskContainerRef} onClick={handleContainerClick} className='bg-cyan-500 w-[35rem] mx-6 sm:mx-0 min-h-[50rem] animate-fade-down animate-once animate-duration-[400ms] animate-normal rounded-lg p-10 overflow-auto my-10'>
          {/* Shows the delete modal inside the task details */}
          {showDeleteTask ? <DeleteTask setShowDeleteTask={setShowDeleteTask} /> : isEditingTask ? <EditTask setIsEditingTask={setIsEditingTask} /> :
            <>
              <div className='flex justify-between mb-10'>
                ...
                <div className='flex items-center gap-2'>
                  ...
                  <Button onClick={() => setIsTaskDetailsOpen(false)} type='cross' />
                </div>
              ...
              <p className='mb-4 w-fit text-slate-600'>{data?.data?.description}</p>
              ...
              {data?.data?.subtasks.map((task) => {
                ...
                return (
                  ...
                  >
                    ...
                    />
                    <p className={`font-semibold text-lg ${status === 'done' ? 'line-through' : ''}`}>{name}</p>
                  </div>
                )
              })}
